...
<?php
    ...
    try{
      ...
      if (isset($_POST['att'])) {
        $course = isset($_POST['whichcourse']) ? $_POST['whichcourse'] : '';

        foreach ($_POST['st_status'] as $i => $st_status) {
            $stat_id = isset($_POST['stat_id'][$i]) ? $_POST['stat_id'][$i] : '';
            $dp = date('Y-m-d');
            ...
            $stmt = $mysqli->prepare("INSERT INTO attendance (stat_id, course, st_status, stat_date) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $stat_id, $course, $st_status, $dp);
            $stmt->execute();
        }
  ...
  <div class="content">
   ...
   <?php
    ...
    if ($mysqli->connect_error) {
     ...
     $result = $mysqli->query("SELECT * FROM students WHERE st_email='$batch' ORDER BY st_id ASC");
